...
{"sample_id": "${SAMPLE}", "tool": "sourmash_gather", "profile": "${PROFILE_CAMI}", "gather_csv": "${GATHER_CSV}", "seqmap": "${SEQMAP}", "index": "${INDEX_FILE}", "query_sig": "${QUERY_SIG}", "ksize": "${KSIZE}", "scaled": "${SCALED}", "top_hits": "${TOP_HITS}", "threads": "${THREADS}", "sourmash_version": "${SOURMASH_VER}"}
EOF
